pro extract_straylight_spectrum_nominal_posneg, cube_name
  ...
  straylight_spectrum_name_pos=strcompress(basename+endname+'_pos.sav', /remove_all)
  straylight_spectrum_name_neg=strcompress(basename+endname+'_neg.sav', /remove_all)
  ...
  if opt eq 0 then begin
    ...
    for order_index=0,5 do begin
      dif_straylight[order_index+2]=median(straylight_spectrum_model[pos[2,order_index]:pos[3,order_index]:2, *], dimension=1)-$
        median(straylight_spectrum_model[pos[0,order_index]:pos[1,order_index]:2, *],dimension=1)
      index_straylight[order_index+2,*]=median(data[pos[2,order_index]:pos[3,order_index]:2, *], dimension=1)-$
        median(data[pos[0,order_index]:pos[1,order_index]:2, *], dimension=1)
    endfor
    ...
    straypos=where((index_straylight[3, *]) gt 5*stddev(data)^2 and therm lt 0.0005 and $
    ...
    if (n_elements(straypos) lt 2) then begin
      print, 'no sufficient "positive" straylight in cube T1_'+endname
    endif else begin
    ...
    STRAYLIGHT_SPECTRUM_MEASURE_POS= meanstray
    STRAYLIGHT_SPECTRUM_MODEL_POS= straylight_spectrum_mod
    abb=where(STRAYLIGHT_SPECTRUM_MODEL_POS le 0.)
    STRAYLIGHT_SPECTRUM_MODEL_POS[abb]=0.
  
  
    save, STRAYLIGHT_SPECTRUM_MEASURE_POS, STRAYLIGHT_SPECTRUM_MODEL_POS, filename=straylight_spectrum_name_pos
    print, ' "positive" straylight extracted for cube T1_'+endname
    
  endelse
  
  ;***NEGATIVE MANIFESTATION
  straypos=where( therm lt 0.0005 and $
    mis_order0 lt -2.*stddev(data)^2 and mis_order3 lt -2.*stddev(data)^2 ); and abs(index_straylight[3, *]) lt 20*stddev(data)^2 )
  if (n_elements(straypos) lt 2) then begin
    print, 'no sufficient "negative" straylight in cube T1_'+endname
  endif else begin
    data_stray=data[*,straypos]
    meanstray=median(data_stray, dimension=2)
    ;p=plot_virtish(meanstray)
  ...
  STRAYLIGHT_SPECTRUM_MEASURE_NEG= meanstray
  STRAYLIGHT_SPECTRUM_MODEL_NEG= straylight_spectrum_mod
  abb=where(STRAYLIGHT_SPECTRUM_MODEL_NEG ge 0.)
  STRAYLIGHT_SPECTRUM_MODEL_NEG[abb]=0.
  
  
  save, STRAYLIGHT_SPECTRUM_MEASURE_NEG, STRAYLIGHT_SPECTRUM_MODEL_NEG, filename=straylight_spectrum_name_neg
  print, ' "negative" straylight extracted for cube T1_'+endname
